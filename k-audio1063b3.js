...
var csstext=".kc-body{display:inline-block;height:32px;margin:1px 5px;}" +
    ...
    ".kc-lrcbody{display:table-cell;overflow:hidden;vertical-align:middle;text-align:center;max-height:25px;line-height:90%}" +
...
function kac(audio,width,lrc) {
    ...
    kaplayer++;
    jready();
    var kbody,bloc,lrcbody,lrctext,lrcli,lrclong,x,xx,xxx,ar,ti,timetoupmin,timetoups,timetoupms,upt,lrcnuber,lrccc,infonuber=0;
    var play,pause,rangep,rangeli,rangebody,cnt,cat,rangepl,soundbody,soundicon,soundbodyleft,soundpl,sound0icon,sound1icon,sound2icon,sound3icon,nosound,v,vv,soundli,soundp,infbody,lrcfilebox,lrctable,$$$show=false;
    var lrcarray=[],lrcpart=[],timetouptime=[];
...
//------------测试版1.0.6.3 build 2-----------------
    ...
    if(lrc){
        {
            $(document).ready(function () {
                ...
                if (ar===undefined&&ti===undefined){ar="k-audio.js";lrcbody.style.fontWeight="bolder";lrcbody.style.fontSize="14px";}
                if (ar&&ti){ar=ar+" - "}
                if (ar===undefined){ar="";}
                if (ti===undefined){ti=""}
                ...
                setInterval(function () {
                    ...
                    lrcnuber=upt.length;
                    if (lrcnuber>0){
                        if (lrccc!==lrcnuber){lrccc=lrcnuber;lrcbody.innerHTML=lrcpart[lrccc+infonuber-1];}}
                },10);
    ...
    function jready() {
        if(window.jQuery||jQuery||$()){kacc(audio,width,lrc);}
        else{console.warn("没有检测到jQuery服务！");}
    }
    function kacc(audio,width,lrc){
    ...
    /*-------------MAIN--------------*/
        ...
        function soundready() {
            ...
            soundbody.id="kc-soundbody"+kaplayer;
        ...
        function whensound() {
            ...
            soundbody.addEventListener("dbclick",function () {
                soundbody.style.left=parseInt(width-96)+"px";
                $$$show=true;
            });
